...
@torch.no_grad()
def create_simplex_constraints(model):
    constraints = []
    for name, param in model.named_parameters():
        n = param.numel()
        if name == 'MLP.v':
                constraint = UnitSimplex(param.numel())
        else:
                constraint = Unconstrained(param.numel()) 
        constraints.append(constraint)
    return constraints
...
class Positivity(Constraint):
    ...
    @torch.no_grad()
    def Jacobian(self,v,n):
        super().Jacobian(v,n)
        d = structure_const()
        J = torch.eye(8) - 2*sqrt(3)*1/torch.norm(v)* torch.einsum('i,jpq,p,q->ij',v,d,n,n) -2*sqrt(3)*torch.einsum('ijk,k->ij',d,n)
        return J
    ...
    def F(self,v,n):
        super().F(v,n)
        d = structure_const()
        f = n + v /torch.norm(v) *(2/3-2/sqrt(3)*torch.einsum('pqr,p,q,r->',d,n,n,n ) )-sqrt(3)*torch.einsum('ijk,j,k->i',d,n,n)
        return f
 
    @torch.no_grad()
    def Newtonstep(self,v,n):
        super().Newtonstep(v,n)
        J = self.Jacobian(v,n).numpy()
        f = self.F(v,n).numpy()
        y = torch.from_numpy(LA.solve(J,-f))
        return y+n 
    ...
    @torch.no_grad()
    def lmo(self,v):
        """Returns a valid coherence vector n minimizing v*n"""
        v = v[:8]
        d = structure_const()
        n =  -2/3*v/torch.norm(v) -2/sqrt(3)*(2/(3*torch.norm(v)))**3*torch.einsum('abc,a,b,c->',d,v,v,v) \
        +sqrt(3)*(2/(3*torch.norm(v)))**2*torch.einsum('ipq,p,q->i',d,v,v )
        epsilon = 0.00001
        it = 1 
        while torch.norm(F(v,n)) > epsilon:
           it = it+1
           if it == 50: raise RecursionError('Newton exceeded recursion limit of 50') 
           n = Newtonstep(v,n)
        return n                                  
...
class UnitSimplex(Constraint):
    """
   """

    def __init__(self, n):
        super().__init__(n)
        self.data_dim = int(sqrt(n+1))
    ...
    def euclidean_project(self, x):
        super().euclidean_project(x)
        #c_re =  1./(self.data_dim )*(torch.eye(self.data_dim)  + torch.einsum('kij,k->ij',self.basis_re ,x)) 
        #c_im =  1./(self.data_dim )*torch.einsum('kij,k->ij',self.basis_im ,x)
        #c_re =  (torch.eye(self.data_dim )/self.data_dim + torch.einsum('kij,k->ij',self.basis_re ,x)) 
        #c_im =  torch.einsum('kij,k->ij',self.basis_im ,x)
        ...
        c_re[idx[0],idx[1]] = x[idx[0],idx[1]]          
        c_re[idx[1],idx[0]] = x[idx[0],idx[1]]          
        c_im[idx_1[0],idx_1[1]] = x[idx_1[1],idx_1[0]]  
        c_im[idx_1[1],idx_1[0]] =-x[idx_1[1],idx_1[0]]  
        
        y = torch.zeros(self.data_dim,self.data_dim)

        c = 1j*c_im.numpy() + c_re.numpy()
        sigma, Q = LA.eigh(c)
        if one_negative( sigma):
                #sigma = simplexion(sigma)
                #sigma = 0.5*(sigma + np.abs(sigma))
                #sigma   = np.array([ max(s,0) for s in sigma],dtype = float)
                sigma   = [ max(s,0) for s in sigma]
                #sigma  /= np.sum(sigma)
                c = np.einsum('ik,k,kl->il',Q,sigma,np.conj(Q.T))
                c_re = np.real(c)
                c_im = np.imag(c)
                y[idx[0],idx[1]] = torch.from_numpy( c_re[idx[0],idx[1]]).float()
                y[idx_1[1],idx_1[0]] = torch.from_numpy (c_im[idx_1[0],idx_1[1]]).float()
                                
        tc_re = torch.zeros(self.data_dim,self.data_dim)
        tc_im = torch.zeros(self.data_dim,self.data_dim)
        tc_re[idx[0],idx[1]] = y[idx[0],idx[1]]          
        tc_re[idx[1],idx[0]] = y[idx[0],idx[1]]          
        tc_im[idx_1[0],idx_1[1]] = y[idx_1[1],idx_1[0]]  
        tc_im[idx_1[1],idx_1[0]] =-y[idx_1[1],idx_1[0]]  
 
        tc = 1j*tc_im.numpy() + tc_re.numpy()
        sigma, Q = LA.eigh(c)
#        print(sigma)
        return y
